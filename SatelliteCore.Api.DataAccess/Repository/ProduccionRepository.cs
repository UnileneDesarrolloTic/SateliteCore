...
namespace SatelliteCore.Api.DataAccess.Repository
{
    public class ProduccionRepository : IProduccionRepository
    {
        ...
        public async Task<IEnumerable<DatosFormatoTransitoPendienteOC>> MostrarOrdenCompraArima( string Item)
        {
            ...
            string sql = "DECLARE @Periodo VARCHAR(6); " +
                         "SELECT TOP 1 @Periodo = Periodo FROM[dbo].[ML_Pronostico] ORDER BY Periodo DESC " +
                         "SELECT RTRIM(b.Item) Item, RTRIM(c.NumeroOrden) NumeroOrden, RTRIM(d.NombreCompleto) Proveedor, b.CantidadPedida Cantidad, b.CantidadRecibida, (b.CantidadPedida - b.CantidadRecibida) PendienteOC, b.FechaPrometida Fecha, DATEDIFF(DAY, c.FechaPrometida, GETDATE()) DiferenciaFecha " +
                         "FROM ML_Pronostico a INNER JOIN WH_OrdenCompraDetalle b WITH(NOLOCK) ON a.ItemSpring = b.Item AND a.Periodo = @Periodo " +
